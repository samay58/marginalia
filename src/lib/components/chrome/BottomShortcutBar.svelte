<script>
   ...
   *   onDone?: () => void
   * }} */
  let { edits, annotations, saveLabel, saveFresh, saveFailed = false, onNotes, onRationale, onAddRef, onUndo, onDone } = $props();
...
<div class="bar">
  <div class="status">
    <span class="count">{edits} {edits === 1 ? 'edit' : 'edits'}</span>
    <span class="count muted">{annotations} {annotations === 1 ? 'rationale' : 'rationales'}</span>
    <StatusLED label={saveLabel} on={saveFresh} warn={saveFailed} />
  </div>
